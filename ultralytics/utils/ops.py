...
def non_max_suppression(
    ...
    Returns:
        (List[torch.Tensor]): A list of length batch_size, where each element is a tensor of
            ...
            (x1, y1, x2, y2, confidence, class, IoU, mask1, mask2, ...).
    """
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        box_dnms = box[i]
        iou_dnms = torch.zeros((box_dnms.size(0), 1), device=prediction.device)
        for index in range(box_dnms.shape[0]):
            current_box = box_dnms[index, :]
            if index == 0:
                continue
            else:
                upper_boxes = box_dnms[:index, :]
            iou_metrics = bbox_iou(current_box, upper_boxes,
                                   xywh=False, GIoU=False, DIoU=False, CIoU=False, eps=1e-7)
            iou_dnms[index] = iou_metrics.max(dim=0).values
        ...
        # output[xi] = x[i]
        output[xi] = torch.cat((x[i], iou_dnms), dim=1)
